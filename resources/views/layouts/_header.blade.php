 ...
 <nav class="navbar navbar-expand-lg navbar-light py-3 py-lg-0 px-0 px-lg-5" >
        ...
        @endphp
          ...
          <form action="{{ route('logout') }}" method="POST" style="display: inline;">
           @csrf
            <button type="submit" class="btn btn-primary px-4"style="background-color:#002244;">Logout</button>
            @if(Auth::user()->is_admin == 0)
            <a href="{{ url('panel/dashboard_user') }}" class="btn btn-primary px-4" style="margin-left: 8px;background-color:#002244;">Dashboard</a>
            @endif
            @if(Auth::user()->is_admin == 1)
            <a href="{{ url('panel/dashboard') }}" class="btn btn-primary px-4" style="margin-left: 8px;background-color:#002244;">Dashboard</a>
            @endif
          </form>
          @endauth
